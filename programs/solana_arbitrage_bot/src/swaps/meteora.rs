...
use dynamic_amm::{self, cpi_accounts::Swap as MeteoraSwapAccounts, program::DynamicAmm};
...
pub struct MeteoraSwap<'info> {
    /// CHECK: Validates by Meteora program
    #[account(mut)]
    ...
    ///CHECK: Validated by Meteora Program
    pub valud_program: AccountInfo<'info>,

    pub meteora_program: Program<'info, DynamicAmm>,
}

impl<'impl> MeteoraSwap<'info> {
    pub fn process_swap(
        &self,
        amount_in: u64,
