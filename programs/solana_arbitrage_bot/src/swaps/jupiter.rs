...
use anchor_lang::solana_program::{instruction::Instruction , program::invoke}; // CPI (Crross Program Invocation)
...
pub struct JupiterSwap<'info> {
    /// CHECK: Validate by Jupiter Program
    #[account(address = JUPITER_PROGRAM_ID.parse::<Pubkey>().unwarp())]
...
impl<'info> JupiterSwap<'info> {
    pub fn process_swap(
        ...
        let mut final_route_data = Vec::with_capacity(route_data.len() + 16); // 16 bytes for the amount_in and minimum_amount_out
